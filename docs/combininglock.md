...
struct LockNode
{
  cpp::atomic<LockNode*> next{nullptr};
  cpp::atomic<bool> available{false};
};

struct MCSLock
{
  // End of queue
  cpp::atomic<LockNode*> last;
};
...
inline void with(MCSLock& lock, F&& f)
{
  ...
  LockNode* prev = lock.last.exchange(&node, cpp::memory_order_acq_rel);
  if (prev != nullptr)
  {
    // Add link to previous element in the queue
    prev->next.store(&node, cpp::memory_order_release);
    // Wait for out turn.
    while (!node.available.load(cpp::memory_order_acquire))
      ;
  }
  ...
  if (node.next.load(cpp::memory_order_acquire) == nullptr)
  {
    auto node_address = &node;
    // No next thread so remove ourselves from the end of the queue
    if (lock.last.compare_exchange_strong(node_address, nullptr, cpp::memory_order_acq_rel))
      return;
    // Wait for next thread to be set as we failed to remove ourselves from the end of the queue.
    while (node.next.load(cpp::memory_order_acquire) == nullptr)
      ;
  }

  // Wake next thread.
  node.next.load(cpp::memory_order_acquire)->available.store(true, cpp::memory_order_release);
}
```
...
{
  cpp::atomic<CombiningLockNode*> next{nullptr};
  cpp::atomic<LockStatus> status{WAITING};
...
{
  CombiningLockNodeTempl<F> node(cpp::forward<F>(f));

  // **************ACQUIRE**************
  // Add ourselves to the end of the queue.
  CombiningLockNode* prev = lock.last.exchange(&node, cpp::memory_order_acq_rel);
  ...
  if (prev != nullptr)
  {
    ...
    // Wait for our turn.
    while (node.status.load(cpp::memory_order_relaxed) == LockStatus::WAITING)
      ;

    // Check if another thread completed our work.
    if (node.status.load(cpp::memory_order_acquire) == LockStatus::DONE)
      return;
  }
  ...
  while (true)
  {
    ...
    auto next = curr->next.load(cpp::memory_order_acquire);
    if (next == nullptr)
      break;

    // Notify thread that we completed its work.
    curr->status.store(LockStatus::DONE, cpp::memory_order_release);
    curr = next;
  }
  ...
  if (lock.last.compare_exchange_strong(curr_address, nullptr, cpp::memory_order_acq_rel))
  {
    curr->status.store(LockStatus::DONE, cpp::memory_order_release);
    return;
  }
  ...
  while (curr->next.load(cpp::memory_order_relaxed) == nullptr)
    ;
  ...
  next->status.store(LockStatus::HEAD, cpp::memory_order_release);
  return;
}
```
