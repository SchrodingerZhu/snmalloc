...
namespace snmalloc
{
  ...
  struct CombiningLock
  {
    ...
    cpp::atomic<CombiningLockNode*> last{nullptr};

    void release()
    {
      flag.store(false, cpp::memory_order_release);
    }
  };
  ...
  class CombiningLockNode
  {
    ...
    cpp::atomic<LockStatus> status{LockStatus::WAITING};
    ...
    {
      status.store(s, cpp::memory_order_release);
    }
    ...
    static void wake(CombiningLockNode* node, LockStatus message)
    {
      ...
      {
        if (
          node->status.exchange(message, cpp::memory_order_acq_rel) ==
          LockStatus::SLEEPING)
        {
    ...
    void wait()
    {
      if constexpr (!use_wait_on_address<Pal>)
      {
        while (status.load(cpp::memory_order_acquire) == LockStatus::WAITING)
          Aal::pause();
      }
      else
      {
        ...
        while (remaining > 0)
        {
          if (status.load(cpp::memory_order_acquire) != LockStatus::WAITING)
            return;
          Aal::pause();
          remaining--;
        }
        ...
        if (status.compare_exchange_strong(
              expected, LockStatus::SLEEPING, cpp::memory_order_acq_rel))
        {
    ...
    SNMALLOC_SLOW_PATH void attach_slow(CombiningLock& lock)
    {
      ...
      auto prev = lock.last.exchange(this, cpp::memory_order_acq_rel);

      if (prev != nullptr)
      {
        // If we aren't the head, link into predecessor
        prev->next.store(this, cpp::memory_order_release);

        // Wait to for predecessor to complete
        wait();

        // Determine if another thread completed our work.
        if (status.load(cpp::memory_order_acquire) == LockStatus::DONE)
          return;
      }
      else
      {
        ...
        while (lock.flag.exchange(true, cpp::memory_order_acquire))
        {
          while (lock.flag.load(cpp::memory_order_relaxed))
          {
            Aal::pause();
          }
        }
      ...
      while (true)
      {
        ...
        auto n = curr->next.load(cpp::memory_order_acquire);
        Aal::prefetch(n);
        ...
        n = curr->next.load(cpp::memory_order_acquire);
        if (n == nullptr)
          break;
      ...
      if (lock.last.compare_exchange_strong(
            curr_c,
            nullptr,
            cpp::memory_order_release,
            cpp::memory_order_relaxed))
      {
  ...
  inline void with(CombiningLock& lock, F&& f)
  {
    ...
    if (SNMALLOC_LIKELY(lock.last.load(cpp::memory_order_relaxed) == nullptr))
    {
      // No one was waiting so low contention. Attempt to acquire the flag
      // lock.
      if (SNMALLOC_LIKELY(
            lock.flag.exchange(true, cpp::memory_order_acquire) == false))
      {
        // We grabbed the lock.
        // Execute the thunk.
        f();
    ...
    // with the queue.
    CombiningLockNodeTempl<F> node(lock, cpp::forward<F>(f));
  }
} // namespace snmalloc
