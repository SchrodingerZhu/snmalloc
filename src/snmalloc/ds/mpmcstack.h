...
namespace snmalloc
{
  ...
  private:
    ...
    racy_read(cpp::atomic<T*>& ptr)
    {
      // reinterpret_cast is required as TSAN still instruments
      // cpp::atomic operations, even if you disable TSAN on
      // the function.
      return *reinterpret_cast<T**>(&ptr);
    }
#else
    static T* racy_read(cpp::atomic<T*>& ptr)
    {
  ...
  public:
    ...
    {
      static_assert(
        cpp::is_same<decltype(T::next), cpp::atomic<T*>>::value,
        "T->next must be an cpp::atomic<T*>");
    ...
    void push(T* first, T* last)
    {
      ...
      {
        auto top = cmp.ptr();
        last->next.store(top, cpp::memory_order_release);
