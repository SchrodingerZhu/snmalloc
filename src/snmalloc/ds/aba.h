...
namespace snmalloc
{
  ...
  public:
    ...
    void init(T* x)
    {
      independent.ptr.store(x, cpp::memory_order_relaxed);
      independent.aba.store(0, cpp::memory_order_relaxed);
    }
...
#  endif
      return Cmp{
        {independent.ptr.load(cpp::memory_order_relaxed),
         independent.aba.load(cpp::memory_order_relaxed)},
        this};
    }
...
#    endif
        ...
        cpp::atomic<Linked>& addr = parent->linked;

        auto result = addr.compare_exchange_weak(
          old, xchg, cpp::memory_order_acq_rel, cpp::memory_order_relaxed);
#  endif
        return result;
      }
...
#  endif
    ...
    T* peek()
    {
      return independent.ptr.load(cpp::memory_order_relaxed);
    }
  };
#else
  /**
  ...
  {
    cpp::atomic<T*> ptr = nullptr;
    cpp::atomic<bool> lock{false};

  public:
    // This method is used in Verona
    void init(T* x)
    {
      ptr.store(x, cpp::memory_order_relaxed);
    }
    ...
    Cmp read()
    {
      while (lock.exchange(true, cpp::memory_order_acquire))
        Aal::pause();
...
#  endif
    ...
    public:
      ...
      ~Cmp()
      {
        parent->lock.store(false, cpp::memory_order_release);
#  if !defined(NDEBUG) && !defined(SNMALLOC_DISABLE_ABA_VERIFY)
        operation_in_flight = false;
#  endif
      }
    ...
    T* peek()
    {
      return ptr.load(cpp::memory_order_relaxed);
    }
  };
#endif
} // namespace snmalloc
