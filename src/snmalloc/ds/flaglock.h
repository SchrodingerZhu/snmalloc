...
namespace snmalloc
{
   ...
   * Wrapper for cpp::atomic_flag so that we can examine
  ...
  struct DebugFlagWord
  {
     ...
     */
    cpp::atomic_bool flag{false};

    constexpr DebugFlagWord() = default;

    template<typename... Args>
    constexpr DebugFlagWord(Args&&... args) : flag(cpp::forward<Args>(args)...)
    {}
  ...
  private:
     ...
     */
    cpp::atomic<ThreadIdentity> owner = ThreadIdentity();
  ...
  public:
    ...
    {
      lock.clear_owner();
      lock.flag.store(false, cpp::memory_order_release);
    }
  };
