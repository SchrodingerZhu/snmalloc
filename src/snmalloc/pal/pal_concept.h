...
namespace snmalloc
{
  ...
  concept IsPAL_static_features = requires() {
    typename cpp::integral_constant<uint64_t, PAL::pal_features>;
  };
  ...
  concept IsPAL_static_sizes = requires() {
    typename cpp::integral_constant<cpp::size_t, PAL::address_bits>;
    typename cpp::integral_constant<cpp::size_t, PAL::page_size>;
  };
   ...
   */
  template<typename PAL>
  concept IsPAL_memops = requires(void* vp, cpp::size_t sz) {
   ...
   */
  template<typename PAL>
  concept IsPAL_reserve = requires(PAL p, cpp::size_t sz) {
   ...
   */
  template<typename PAL>
  concept IsPAL_reserve_aligned = requires(cpp::size_t sz) {
