...
#  error Unsupported platform
  ...
  static inline typename cpp::enable_if_t<
  ...
  template<
    ...
    typename T,
    SNMALLOC_CONCEPT(capptr::IsBound) B>
  static SNMALLOC_FAST_PATH typename cpp::enable_if_t<
    aal_supports<StrictProvenance, AAL>,
    CapPtr<T, capptr::user_address_control_type<B>>>
  capptr_to_user_address_control(CapPtr<T, B> p)
  {
...
#  endif
   ...
   *  - Strings (anything convertible to `cpp::string_view`), typically string
  ...
  {
    MessageBuilder<BufferSize> msg{cpp::forward<Args>(args)...};
