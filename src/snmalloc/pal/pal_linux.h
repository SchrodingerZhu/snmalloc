...
#  ifdef SYS_getrandom
      static cpp::atomic_bool syscall_not_working = false;
      ...
      if (false == syscall_not_working.load(cpp::memory_order_relaxed))
      {
        ...
        if (SNMALLOC_UNLIKELY(target != current))
        {
          ...
          syscall_not_working.store(true, cpp::memory_order_relaxed);
        }
        else
        {
          return result;
...
#  ifdef SNMALLOC_HAS_LINUX_FUTEX_H
    ...
    template<class T>
    static void notify_one_on_address(cpp::atomic<T>& addr)
    {
      static_assert(
        sizeof(T) == sizeof(WaitingWord) && alignof(T) == alignof(WaitingWord),
        "T must be the same size and alignment as WaitingWord");
    ...
    template<class T>
    static void notify_all_on_address(cpp::atomic<T>& addr)
    {
      static_assert(
        sizeof(T) == sizeof(WaitingWord) && alignof(T) == alignof(WaitingWord),
        "T must be the same size and alignment as WaitingWord");
