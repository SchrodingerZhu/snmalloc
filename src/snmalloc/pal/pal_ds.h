...
namespace snmalloc
{
  ...
  class PalList
  {
     ...
     */
    cpp::atomic<T*> elements{nullptr};

    static_assert(
      std::is_same<decltype(T::pal_next), cpp::atomic<T*>>::value,
  ...
  {
    cpp::atomic<PalNotificationObject*> pal_next = nullptr;
  ...
  class PalTimerObject
  {
    ...
    cpp::atomic<PalTimerObject*> pal_next;
  ...
  public:
    ...
    {
      static cpp::atomic_bool lock{false};

      // Deduplicate calls into here, and make single threaded.
      if (lock.exchange(true, cpp::memory_order_acquire))
        return;
      ...
      lock.store(false, cpp::memory_order_release);
    }
  };
} // namespace snmalloc
