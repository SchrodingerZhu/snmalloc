...
namespace snmalloc
{
   ...
   * - `get_bits(Contents)`, `set_bits(Contents, uint8_t)` (bit0 = parity)
  ...
  class RBTree
  {
    ...
    static bool parity(K n)
    {
      if (is_null(n))
        return true;
      return (Rep::get_bits(n) & ParityMask) != 0;
    }

    static void set_parity(K n, bool p)
    {
      if (!is_null(n))
        Rep::set_bits(n, p ? 1 : 0);
    }

    static void toggle_parity(K n)
    {
      if (!is_null(n))
        Rep::set_bits(n, static_cast<uint8_t>(Rep::get_bits(n) ^ 1));
    }
