...
namespace snmalloc
{
  ...
  public:
     ...
     */
    using LocalState = cpp::conditional_t<
  ...
  private:
     ...
     */
    SNMALLOC_REQUIRE_CONSTINIT
    inline static cpp::atomic<bool> initialised{false};
    ...
    SNMALLOC_SLOW_PATH static void ensure_init_slow()
    {
      ...
      with(initialisation_lock, [&]() {
        ...
        initialised.store(true, cpp::memory_order_release);
      });
    }
  ...
  public:
    ...
    {
      if (SNMALLOC_LIKELY(initialised.load(cpp::memory_order_acquire)))
        return;
